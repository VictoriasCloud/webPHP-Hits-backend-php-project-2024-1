...
function getAlistOfMedicalInspectionsForConsultation() {
    ...
    $getInspectionQuery = "SELECT id FROM inspection";
    $inspectionResult = $Link->query($getInspectionQuery);
    if ($inspectionResult->num_rows > 0) {
        // Используем цикл для обхода каждой строки результата запроса
        while ($row = $inspectionResult->fetch_assoc()["id"]) {
